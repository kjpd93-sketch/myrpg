...
import { Character, CLASSES, RACES } from './modules/character.js';
...
class App {
  ...
  setupCharacterCreation() {
    ...
    document.querySelectorAll('.choice-card[data-type="race"]').forEach(card => {
      card.addEventListener('click', () => selectChoice('race', card.dataset.key, card));
    });
    document.querySelectorAll('.choice-card[data-type="class"]').forEach(card => {
      card.addEventListener('click', () => selectChoice('class', card.dataset.key, card));
    });
  ...
  setupEventListeners() {
    // Togglen des Charakterblatts in der Sidebar auf kleinen Bildschirmen
    const toggleBtn = document.getElementById('btn-toggle-character');
    toggleBtn.addEventListener('click', () => {
      this.showScreen('screen-talents');
    });

    const closeTalentsBtn = document.getElementById('btn-close-talents');
    closeTalentsBtn.addEventListener('click', () => {
      this.showScreen('screen-village');
    });
  ...
  updateCharacterSidebar() {
    ...
    // Stats
    document.getElementById('stat-strength').innerText = this.player.stats.strength;
    document.getElementById('stat-agility').innerText = this.player.stats.agility;
    document.getElementById('stat-intellect').innerText = this.player.stats.intellect;
    document.getElementById('stat-stamina').innerText = this.player.stats.stamina;

    const armor = this.player.getArmor();
    const redPct = Math.round(this.player.getDamageReduction() * 100);
    document.getElementById('stat-armor').innerText = armor;
    document.getElementById('stat-reduction').innerText = `${redPct}%`;

    document.getElementById('stat-damage').innerText = this.player.getPhysicalDamage();
    document.getElementById('stat-spellpower').innerText = this.player.getSpellPower();
    ...
    if (this.player.inventory.length === 0) {
      invList.innerHTML = `<div class="placeholder-text" style="font-size:0.8rem; text-align:center; padding:10px; color:var(--text-muted);">Dein Beutel ist leer.</div>`;
    } else {
      invList.innerHTML = this.player.inventory.map(item => {
        ...
        return `
          <div class="item-card ${typeClass}" data-id="${item.uniqueId}">
            <div class="item-info">
              <span class="item-title">${item.name}</span>
              <span class="item-desc">${statText || item.description || ''}</span>
            </div>
          </div>
        `;
      }).join('');

      // Click listener to equip or use item
        ...
        card.addEventListener('click', () => {
          ...
          if (item.type === 'consumable') {
            ...
            if (res.error) {
              ...
              alert(res.text);
              this.updateUI();
              if (this.currentScreen === 'screen-combat') this.renderCombat();
              this.saveGameToLocalStorage();
            }
          } else {
  ...
  renderDungeonCrawler() {
    ...
    document.getElementById('dungeon-progress-text').innerText = `Raum ${this.dungeonRun.currentRoomIndex} / ${this.dungeonRun.maxRooms}`;
    const pct = (this.dungeonRun.currentRoomIndex / this.dungeonRun.maxRooms) * 100;
    ...
    if (room.type === 'combat' || room.type === 'boss') {
      ...
      document.getElementById('btn-room-fight').onclick = () => {
        this.combat = new Combat(this.player, room.enemyKey);
        this.showScreen('screen-combat');
      };
    } 
    else if (room.type === 'safe') {
      if (room.resolved) {
        actionDiv.innerHTML = `<button class="premium-btn" id="btn-room-proceed">Fortfahren</button>`;
        document.getElementById('btn-room-proceed').onclick = () => {
          this.dungeonRun.nextRoom();
        ...
        document.getElementById('btn-room-rest').onclick = () => {
          ...
          alert(`Du rastest eine Weile. (+${reg.hpRegen} HP, +${reg.resRegen} Mana)`);
          this.updateUI();
          this.renderDungeonCrawler();
        };
      }
      ...
      if (room.resolved) {
        ...
        if (this.dungeonRun.isPlayerDead) {
          ...
          actionDiv.innerHTML = `<button class="premium-btn" id="btn-room-proceed">Weitergehen</button>`;
          document.getElementById('btn-room-proceed').onclick = () => {
            this.dungeonRun.nextRoom();
            this.renderDungeonCrawler();
          };
        ...
        if (room.eventType === 'chest' || room.eventType === 'trap') {
          actionDiv.innerHTML = `<button class="premium-btn" id="btn-room-resolve-event">${room.eventType === 'chest' ? 'Truhe öffnen' : 'Falle entschärfen'}</button>`;
  ...
  renderCombat() {
    ...
    // FEIND UI
    document.getElementById('combat-enemy-name').innerText = this.combat.enemy.name;
    document.getElementById('combat-enemy-lvl').innerText = `Lvl ${this.combat.enemy.level}`;
    const enemyHpPct = (this.combat.enemy.currentHp / this.combat.enemy.maxHp) * 100;
    document.getElementById('combat-enemy-hp-bar').style.width = `${enemyHpPct}%`;
    document.getElementById('combat-enemy-hp-text').innerText = `${this.combat.enemy.currentHp} / ${this.combat.enemy.maxHp}`;
    
    document.getElementById('combat-enemy-effects').innerHTML = this.combat.enemy.debuffs.map(d => `
      <span class="effect-badge" style="color:var(--text-red)">${d.name} (${d.duration}r)</span>
    `).join('') + this.combat.enemy.buffs.map(b => `
      <span class="effect-badge" style="color:#28a745">${b.name} (${b.duration}r)</span>
    `).join('');

    // SPIELER UI
    document.getElementById('combat-player-name').innerText = this.player.name;
    document.getElementById('combat-player-lvl').innerText = `Lvl ${this.player.level}`;
    
    const pAv = document.getElementById('combat-player-avatar-icon');
    if (this.player.classKey === 'KRIEGER') pAv.innerText = '⚔️';
    else if (this.player.classKey === 'PALADIN') pAv.innerText = '🛡️';
    else if (this.player.classKey === 'MAGIER') pAv.innerText = '🔥';
    else if (this.player.classKey === 'PRIESTER') pAv.innerText = '✨';

    const playerHpPct = (this.player.currentHp / this.player.maxHp) * 100;
    document.getElementById('combat-player-hp-bar').style.width = `${playerHpPct}%`;
    document.getElementById('combat-player-hp-text').innerText = `${this.player.currentHp} / ${this.player.maxHp}`;

    // Spieler-Ressourcen
    const resBar = document.getElementById('combat-player-resource-bar');
    const resText = document.getElementById('combat-player-resource-text');
    const resContainer = document.getElementById('combat-player-resource-container');
    ...
    document.getElementById('combat-player-effects').innerHTML = this.player.debuffs.map(d => `
      <span class="effect-badge" style="color:var(--text-red)">${d.name} (${d.duration}r)</span>
    `).join('') + this.player.buffs.map(b => `
      <span class="effect-badge" style="color:#28a745">${b.name} (${b.duration}r)</span>
    `).join('') + (this.player.shield ? `<span class="effect-badge" style="color:cyan">Schild: ${this.player.shield} HP</span>` : '');
    ...
    skillsGrid.querySelectorAll('button').forEach(btn => {
      btn.onclick = () => {
        ...
        const res = this.combat.executePlayerTurn(skill);
    ...
    if (this.combat.victory) {
      ...
      this.combat.lootEarned.forEach(loot => {
        if (loot.isMaterial) {
          ...
          // Quest-Fortschritt für Loot sammeln
      ...
      const monsterName = this.combat.enemy.name;
      const isBoss = this.combat.enemy.isBoss || false;
      const questUpdates = updateQuestKills(this.player, monsterName, isBoss);
      ...
      this.showCombatResultOverlay(true, `Du hast '${monsterName}' erfolgreich bezwungen!`, rewardText);
    } else {
      this.showCombatResultOverlay(false, `Du bist im Kampf gegen '${this.combat.enemy.name}' gefallen.`);
    }
  }
